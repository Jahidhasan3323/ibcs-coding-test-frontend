    ...
    <form class="auth-form" @submit.prevent="register()">
      ...
      <div class="form-group">
        <div class="form-label-group">
          <input v-model="form.name" type="text" class="form-control" placeholder="name" autofocus=""> <label
        >Name</label>
        </div>
      </div><!-- /.form-group -->
      <div class="form-group">
        <div class="form-label-group">
          <input v-model="form.email" type="email" class="form-control" placeholder="email" autofocus=""> <label
        >Email</label>
          <div v-for="email in errors['email']" class="alert alert-danger m-0 mt-1">{{ email }}</div>
        </div>
      </div><!-- /.form-group -->
      <div class="form-group">
        <div class="form-label-group">
          <input v-model="form.phone" type="text" class="form-control" placeholder="phone no" autofocus=""> <label
        >Phone No</label>
          <div v-for="phone in errors['phone']" class="alert alert-danger m-0 mt-1">{{ phone }}</div>
        </div>
      </div><!-- /.form-group -->
      ...
      <div class="form-group">
        <div class="form-label-group">
          <input v-model="form.password" type="password" class="form-control" placeholder="Password"> <label
        >Password</label>
          <div v-for="password in errors['password']" class="alert alert-danger m-0 mt-1">{{ password }}</div>
        </div>
      </div><!-- /.form-group -->
      <div class="form-group">
        <div class="form-label-group">
          <input v-model="form.password_confirmation" type="password" class="form-control" placeholder="Password"> <label
  ...
  methods: {
    register() {
      ApiService.post('/register', this.form).then((res) => {
        ...
        if (queryParam && queryParam.from_url == 'checkout') {
        ...
        let errorList = errors.response?.data;
        this.errors = errorList ?? {};
        _.forEach(errorList, (items) => {
          items.forEach((err) => {
          })
        });
      });
